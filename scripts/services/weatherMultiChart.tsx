...
const WeatherMultiChart: React.FC<Props> = ({ latitude, longitude, viewMode }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (viewMode === 'hourly' && Array.isArray((forecast as any).forecastHours)) {
          setData((forecast as any).forecastHours);
        } else if (viewMode === 'daily' && Array.isArray((forecast as any).forecastDays)) {
  ...
  if (!Array.isArray(data) || data.length === 0) {
    return <Text style={{ margin: 20, color: 'red' }}>No forecast data available</Text>;
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.chartBox}>
        <Text style={styles.chartTitle}>{emojiMap.windDirection} Wind Direction</Text>
        <ScrollView horizontal>
      ...
      {viewMode === 'hourly' && renderChart('🌫️ Visibility (km)', visibility, 'mediumpurple')}
    </View>
  );
};
